...
<script>
	...
	function inputJu(obj){
		...
		if(chkDate.getDay().toString() == '0' || chkDate.getDay().toString() == '6'){ // 일요일 또는 토요일
			...
			if(number.length == 0){
				document.getElementById('checkYoilValid').innerText = "";
				document.getElementById('checkYoilInValid').innerText = "";
			}
				...
				if(number.length >= 2){ // 생년월일이 빠르게 3자리가 입력되었을 때 메시지가 안 뜨는 오류 수정 
					if(number.substr(1,1) == chkDate.getDay()){
						document.getElementById('checkYoilInValid').innerText = "";
						document.getElementById('checkYoilValid').innerText = number.substr(0,2) + " 년생은 구매 가능합니다.";
					}
					else if((number.substr(1,1)*1+5).toString().substr(1,1) == chkDate.getDay()){
						document.getElementById('checkYoilInValid').innerText = "";
						document.getElementById('checkYoilValid').innerText = number.substr(0,2) + " 년생은 구매 가능합니다.";
					}
					else{
						document.getElementById('checkYoilValid').innerText = "";
						document.getElementById('checkYoilInValid').innerText = number.substr(0,2) + " 년생은 구매할 수 없습니다.";
					}
				}
					return number;
			}
...
<body>
	...
	<div id="search-cont">
		<span id="yoil"></span>
		<script type="text/javascript"> <!-- 요일 처리 스크립트 -->
			var d = new Date();
		...
		<form action="sellProc.jsp" method="post" name="sellForm">
